/**
 * AEGIS Full End-to-End Test with ACE Validation (V2)
 *
 * This script:
 * 1. Deploys a fresh MockKeystoneForwarder
 * 2. Updates the vault to use this forwarder temporarily
 * 3. Runs the full ACE test
 * 4. Restores original forwarder (optional)
 *
 * Flow:
...
async function simulateAgentResponses(jobId, userPrompt, vaultAddress) {
...
async function main() {
  ...
  console.log("AEGIS Full End-to-End Test with ACE Validation (V2)");
  console.log("=".repeat(70));
  console.log("Deployer:", deployer.address);
  console.log();

  // ========================================
  // STEP 0: Deploy fresh MockKeystoneForwarder
  // ========================================
  console.log("STEP 0: Deploy Fresh MockKeystoneForwarder");
  ...
  console.log("  New MockKeystoneForwarder:", forwarderAddress);

  // Update vault to use our forwarder
  ...
  const originalForwarder = await vault.getForwarderAddress();
  console.log("  Original Vault Forwarder:", originalForwarder);

  await vault.setForwarderAddress(forwarderAddress);
  console.log("  [OK] Vault forwarder updated to MockKeystoneForwarder");
  console.log();

  // Get other contract instances
  const policyEngine = await ethers.getContractAt("ACEPolicyEngine", ADDRESSES.ACE_ENGINE, deployer);
  ...
  const agentResponses = await simulateAgentResponses(jobId, userPrompt, ADDRESSES.VAULT);
  ...
  const tx2 = await mockForwarder.deliverReportSimple(ADDRESSES.VAULT, report);
  ...
  // Update registry forwarder to our MockForwarder
  const registryContract = await ethers.getContractAt(
    ["function setForwarderAddress(address) external", "function getForwarderAddress() external view returns (address)"],
    ADDRESSES.REGISTRY,
    deployer
  );
  const originalRegistryForwarder = await registryContract.getForwarderAddress();
  await registryContract.setForwarderAddress(forwarderAddress);
  ...
  for (const agentId of consensusResult.agentIds) {
    const rewardReport = ethers.AbiCoder.defaultAbiCoder().encode(
      ...
      [4, agentId, ethers.parseEther("10"), 5]
    );

    try {
      const tx = await mockForwarder.deliverReportSimple(ADDRESSES.REGISTRY, rewardReport);
      await tx.wait();
  ...
  // Restore forwarders
  console.log("STEP 10: Cleanup - Restore Original Forwarders");
  console.log("-".repeat(50));
  await vault.setForwarderAddress(originalForwarder);
  await registryContract.setForwarderAddress(originalRegistryForwarder);
  console.log("  [OK] Original forwarders restored");
  console.log();
  ...
  console.log("  1. [✓] Deployed MockKeystoneForwarder for testing");
  console.log("  2. [✓] User created strategy job with natural language prompt");
  console.log("  3. [✓] CRE queried agents via ConfidentialHTTP (simulated)");
  console.log("  4. [✓] Consensus reached among agents");
  console.log("  5. [✓] ACE PolicyEngine validated execution");
  console.log("  6. [✓] CRE delivered report via forwarder");
  console.log("  7. [✓] Vault executed atomic DeFi operation (Uniswap swap)");
  console.log("  8. [✓] Rewards sent to consensus agents");
  console.log("  9. [✓] Original forwarders restored");
  console.log();
